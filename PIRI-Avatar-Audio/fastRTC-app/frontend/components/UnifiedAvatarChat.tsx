...
export function EnhancedVRMAvatarChat() {
  ...
  const initThreeJSWithVRM = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "⚠️ VRM support not available, falling back to GLB:",
          vrmError
        );
  ...
  const applyVRMBlendShapes = useCallback(
    (blendShapes: Record<string, number>) => {
      ...
      if (!vrm) return; // nothing loaded

      const { expressionManager, scene } = vrm;

      if (expressionManager) {
        for (const [shapeName, rawValue] of Object.entries(blendShapes)) {
        ...
        scene.traverse((child: any) => {
          if (
            ...
            typeof child.morphTargetDictionary !== "object"
          ) {
            return;
          }

          // Smooth decay all existing influences
          for (let i = 0; i < child.morphTargetInfluences.length; i++) {
            child.morphTargetInfluences[i] *= 0.9;
          }
          console.log("Applying blend shapes to mesh:", child.name);

          // Apply each new blend shape
          for (const [shapeName, rawValue] of Object.entries(blendShapes)) {
            const index = child.morphTargetDictionary[shapeName];
            if (index === undefined) continue;

            const clamped = Math.min(Math.max(rawValue, 0), 1);
            if (clamped < 0.001) continue;

            const prev = child.morphTargetInfluences[index] || 0;
            child.morphTargetInfluences[index] = prev + (clamped - prev) * 0.3;

            console.log(
              `Setting morph target: ${shapeName} = ${child.morphTargetInfluences[
                index
              ].toFixed(3)}`
            );
          }
        ...
        // No expressionManager _and_ no scene to traverse
        console.warn(
          "VRM has neither an expressionManager nor a scene to modify."
        );
      }
    },
    []
  ...
  // works
  const findDominantBlendShape = useCallback(
    (blendShapes: Record<string, number>): string => {
  ...
  const initEnhancedAIChatSSE = useCallback(() => {
    ...
    eventSource.addEventListener("tts_response", (event) => {
      console.log("Received TTS response:", event.data);
      const data = JSON.parse(event.data);
      setChatMessages((prev) => [
        ...prev,
        { type: "llm", text: data.text, timestamp: Date.now() },
      ]);
      ...
      if (data.audio_b64) {
        ...
        console.error("No audio_b64 in TTS response");
      }

      // 3) Schedule viseme updates in lockstep with the audio
      if (Array.isArray(data.visemes)) {
        data.visemes.forEach((v: EnhancedVisemeData) => {
          setTimeout(() => {
            avatarWebSocketRef.current?.send(
              JSON.stringify({
                type: "update_viseme",
                phoneme: v.viseme,
      ...
      try {
        ...
        if (data.type == "tts_chunk") {
          ...
          if (data.audio_b64) {
            ...
            player
              .play()
              .then(() => console.log("✅ Playing TTS chunk"))
              .catch((err) => console.error("Audio play failed:", err));
          } else {
            console.error("No audio_b64 in TTS chunk");
          }
  ...
  const handleDeviceChange = useCallback(
    (deviceId: string, type: "input" | "output") => {
      ...
      if (type === "input") {
        ...
        if (
          ...
          try {
            (audioRef.current as any).setSinkId(deviceId).catch((err: any) => {
      ...
      <div className="relative z-10 h-full flex">
        ...
        <div className="flex-1 bg-black/20 backdrop-blur-sm border-r border-white/10">
          ...
          <div className="bg-black/30 backdrop-blur-sm p-4 border-b border-white/10">
            ...
            <p className="text-sm text-gray-300">
              Advanced VRoid Facial Animation
            </p>
          </div>
            ...
            <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-sm p-3 rounded-lg text-white">
              ...
              <div className="space-y-1 text-xs">
                ...
                <div>
                  Avatar:{" "}
                  <span
                    className={vrmLoaded ? "text-green-400" : "text-red-400"}
                  >
                ...
                <div>
                  ...
                  <span className="text-yellow-400">
                    {Object.keys(currentBlendShapes).length}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="flex-1 bg-black/20 backdrop-blur-sm flex flex-col">
          ...
          <div className="bg-black/30 backdrop-blur-sm p-4 border-b border-white/10">
            ...
            <p className="text-sm text-gray-300">
              Real-time Speech & Chat with VRM Emotions
            </p>
          </div>
              ...
              {chatMessages.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-lg font-medium text-gray-300">
                    Start a VRM conversation
                  </p>
                  <p className="text-sm mt-1">
                    Click the microphone to speak with enhanced VRM facial
                    animation
                  </p>
                </motion.div>
              ) : (
                chatMessages.map((msg, idx) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className={`flex ${
                      msg.type === "stt" ? "justify-end" : "justify-start"
                    } mb-3`}
                  >
                    <div
              ...
              {isRecording ? (
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-blue-400">
                    ✨ Advanced VRoid visemes • Emotional expressions •
                    Real-time lip sync
                  </div>
                </div>
              )}
          ...
          <div className="border-t border-white/10 p-2 bg-black/20">
            <div className="flex justify-between items-center text-xs text-gray-400">
              <div className="flex items-center gap-3">
                <span>
                  VRM Shapes: {Object.keys(currentBlendShapes).length}
                </span>
              ...
              <div className="flex items-center gap-2">
                ...
                <div
                  className={`w-2 h-2 rounded-full ${
                    vrmLoaded && avatarStatus === "connected"
                      ? "bg-green-400"
                      : "bg-yellow-400"
                  }`}
                ></div>
