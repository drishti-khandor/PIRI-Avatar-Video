...
# ── OVR-to-VRM expression remap ──────────────────────────────────────────────
# The extractor still spits out the old VRoid 0.x names; the viewer expects
# the new VRM 1.x names that appear in the browser’s “Available expressions”.
_OVR2VRM = {
    "Fcl_MTH_A":      "aa",
    "Fcl_MTH_E":      "e",
    "Fcl_MTH_I":      "ih",
    ...
    "Fcl_MTH_U":      "ou",
    # optional helpers
    # "Fcl_MTH_Close":  "mouthOpen",   # drives jaw-open
    # "Fcl_MTH_Neutral": None,         # ignore
    # "Fcl_MTH_Small":  None,
    # "Fcl_MTH_Large":  None,
}

_DEAD_ZONE = 0.02           # ignore tiny weights to cut log spam


def _remap_blend_shapes(src: dict[str, float]) -> dict[str, float]:
    """Translate extractor keys → VRM keys; fold duplicates; no mouthOpen."""
    dst: dict[str, float] = {}
    for key, val in src.items():
        if val < _DEAD_ZONE:
            continue
        vrm_key = _OVR2VRM.get(key)       # map old key → new key
        if not vrm_key:                   # unmapped -- skip silently
            continue
        # add weights that map to the same VRM key
        dst[vrm_key] = round(dst.get(vrm_key, 0.0) + float(val), 3)
    return dst
...
class VRMAvatarController:
    ...
    async def process_audio_chunk(self, audio_chunk, original_sample_rate: int):
        ...
        try:
            ...
            raw_shapes = self.ovr_extractor.visemes_to_blend_shapes(visemes)
            blend_shapes = _remap_blend_shapes(raw_shapes)
            if not blend_shapes:
