...
class AudioProcessor:
    ...
    def _initialize_models(self):
        ...
        if self.forcealign_available:
            ...
            logger.warning("ForceAlign not available. Will use fallback viseme generation.")
    ...
    def _phoneme_to_viseme(self, phoneme: str) -> str:
        ...
        phoneme_to_viseme_map = {
            ...
            'G': 'KK', 'K': 'KK', 'NG': 'NG',
            'S': 'SS', 'Z': 'ZZ',
            'SH': 'SH', 'ZH': 'SH', 'CH': 'CH', 'JH': 'JH',
            'Y': 'YY', 'W': 'WW', 'R': 'RR', 'HH': 'HH',
        ...
        try:
            ...
            try:
                ...
                for phoneme_alignment in phoneme_alignments:
                    ...
                    clean_phoneme = ''.join(c for c in raw_phoneme if c.isalpha()).upper()
            ...
            logger.error(f"Error extracting visemes with ForceAlign: {e}")
            # Fall back to simple viseme generation
            return self.generate_fallback_visemes(text, len(audio_data) / sample_rate)
    
    def generate_fallback_visemes(
        self, 
        text: str, 
        duration: float
    ) -> List[VisemeData]:
